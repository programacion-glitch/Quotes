    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for k, v in data_map.items():
                 debug_dump[k] = str(v)
            with open("debug_text_keys.json", "w") as f:
                json.dump(debug_dump, f, indent=4)
            # END DEBUG DUMP
            ...
            extracted = {
                ...
                "coverages": {
                     "radius_of_operation": get_checkbox_group(data_map, {
                         '134': '0-100', '135': '101-200', '136': '201-300', 
                         '137': '301-500', '138': 'Unlimited'
                     }),
                     "auto_liability_limits": get_checkbox_group(data_map, {
                         '139': '$500K CSL', '140': '$750K CSL', '141': '$1M CSL', '142': 'Others'
                     }),
                     "general_liability": get_checkbox_group(data_map, {
                         '148': '1,000,000', '149': '2,000,000'
                     }), 
                     "physical_damage_deductible": get_checkbox_group(data_map, {
                         '151': '1,000', '152': '2,500'
                     }),
                     "cargo_limit": get_checkbox_group(data_map, {
                         '153': '$100,000', '154': '$250,000', '155': 'Others'
                     }),
                     "refrigeration_breakdown": get_checkbox_bool(data_map, '158'),
                     "pip_med": get_checkbox_bool(data_map, '144'),
                     "um_uim": get_checkbox_bool(data_map, '146'),
                     "non_trucking_liability_limit": get_val(data_map, '150'),
                     "trailer_interchange_limit": get_val(data_map, '159'),
                     "additional_coverages": get_val(data_map, '160'),
                     "cargo_limit_other": get_val(data_map, '156')
                }
            }
            ...
            while True:
                ...
                if has_data:
                    ...
                    # Filter out if mostly empty (sometimes has_data is triggered by empty string? get_val handles lists)
...
def get_checkbox_group(data_map, mapping):
    # Returns the value of the first checked box found in the mapping
    for key, output_val in mapping.items():
        val = get_val(data_map, key)
        # Checkbox values can be '/On', 'Yes', 'True', or specific export values
        # Assume any non-None value that isn't Explicitly 'Off' means checked
        if val:
             v_str = str(val)
...
def get_checkbox_bool(data_map, key):
    val = get_val(data_map, key)
...
if __name__ == "__main__":
    ...
    output_json = f"extracted_{pdf_file.replace('.pdf', '')}.json".replace(" ", "_")
    output_json = output_json.rsplit('\\', 1)[-1] # Ensure filename only if path passed
