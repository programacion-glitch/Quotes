...
def extract_quote_data(pdf_path, output_path):
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # DEBUG PRINT
            print("--- DATA MAP DEBUG ---")
            for k, v in data_map.items():
                if k.isdigit() and int(k) > 130:
                    print(f"Key: {k} | Val: {v} | Type: {type(v)}")
            print("--- END DATA MAP DEBUG ---")
            ...
            extracted = {
                ...
                "coverages": {
                     ...
                     "refrigeration_breakdown": get_checkbox_bool(data_map, '156')
                }
            }
...
def get_checkbox_group(data_map, mapping):
    ...
    for key, output_val in mapping.items():
        ...
        # 'S\xed' decodes to start with 'S' usually or just 'S'
        if val:
             # loose check
             v_str = str(val)
             if '/On' in v_str or 'Yes' in v_str or 'S\xed' in v_str or 'S\xed' in v_str or 'Si' in v_str or 'On' == v_str or v_str.startswith('/S') or v_str.startswith('S'):
                return output_val
    return None
...
def get_checkbox_bool(data_map, key):
    ...
    v_str = str(val)
    return '/On' in v_str or 'Yes' in v_str or 'S\xed' in v_str or 'Si' in v_str or 'On' == v_str or v_str.startswith('/S') or v_str.startswith('S')
