...
class EmailSender:
    ...
    def send_to_mga(
        ...
        """
        Send email to MGA with attachments from memory.
        
        Args:
            to_email: MGA email address
            subject: Email subject (original subject)
            body: Email body (extracted quote portion)
            attachments: Dict mapping filename to attachment dict with 'data' key
        ...
        try:
            ...
            # Add attachments from memory
            for doc_type, att_data in attachments.items():
                filename = att_data.get('filename', f'{doc_type}.pdf')
            ...
            print(f"✓ Email sent to MGA: {to_email}" + (f" (CC: {cc_email})" if cc_email else ""))
            return True
            
        except Exception as e:
            print(f"✗ Failed to send to MGA {to_email}: {e}")
            return False
