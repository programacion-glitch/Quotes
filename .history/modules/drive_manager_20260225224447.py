...
class DriveManager:
    ...
    def _authenticate(self):
        ...
        try:
            ...
            # This solves the "StorageQuotaExceeded" error by acting on behalf of the real user
            admin_email = os.getenv("EMAIL_FROM") # Or whatever real user email we want to use
            if admin_email:
                try:
                    ...
                    print(f"  Drive: Authenticated as {admin_email} (Impersonation)")
                    return service
                except Exception as del_e:
                    print(f"  Drive: Impersonation failed, falling back to Service Account: {del_e}")
                    
            # Fallback to normal service account
    ...
    def upload_files_for_client(self, business_name: str, usdot: str, attachments: List[Dict]) -> bool:
        ...
        for att in attachments:
            ...
            try:
                ...
                # 3. Transfer Ownership to avoid quota limits
                # Service accounts have 0GB quota. If we don't transfer ownership or use a Shared Drive,
                # uploading large files will fail.
                admin_email = os.getenv("EMAIL_FROM", "programacion@h2oins.com")
                if file_id and admin_email:
                    try:
                        permission = {
                            'type': 'user',
                            'role': 'writer',
                        ...
                        print(f"    ✓ Shared with {admin_email}")
                    except Exception as share_e:
                        print(f"    ⚠️ Could not share file: {share_e}")
